...
This project has specialist subagents in `.claude/agents/` for sales and marketing tasks: `content-seo`, `outbound-sdr`, `scientific-community`, `inbound-rfq`, `customer-success`, `competitive-intel`. Delegate to the matching one when a task fits its description rather than doing the work in the main conversation — each keeps its output type isolated and consistent with this positioning doc. Slash commands in `.claude/commands/` wrap the common ones — see `GTM-QUICKSTART.md`.
